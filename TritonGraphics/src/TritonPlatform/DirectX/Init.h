...
NAMESPACE_BEGIN
	...
	// Construct dx context
	DXContext::DXContext(const Triton::AppSettings& appSettings)
		: m_appNameSTD(appSettings.WindowTitle)
	{
		...
		wnd->m_hasWindow = appSettings.hasWindow;
		wnd->m_hwnd = appSettings.hHwnd;
	...
	inline bool Triton::PType::DXContext::init_additional()
	{
		HRESULT result;
		...
		auto[width, height] = dx_window()->getWindowSize();
		...
		swapChainDesc.OutputWindow = dx_window()->m_hwnd;
		...
		rendrr->m_projectionMatrix = rendrr->recalc_projection();
		rendrr->m_orthoMatrix = DirectX::XMMatrixOrthographicLH((float)width, (float)height, rendrr->nearPlane, rendrr->farPlane);
		rendrr->m_worldMatrix = DirectX::XMMatrixIdentity();
		
		rendrr->m_deviceContext = m_deviceContext;
		rendrr->m_renderTargetView = m_renderTargetView;
		rendrr->m_depthStencilView = m_depthStencilView;
	...
	void Triton::PType::DXContext::setViewPort(int x, int y, int width, int height)
	{
		D3D11_VIEWPORT viewport;
		viewport.Width = (float)width;
		viewport.Height = (float)height;
	...
	inline void Triton::PType::DXContext::fillPacket(PlatformObject* packet)
	{
		auto dxPacket = OBJECT_AS(DXPacket, packet);

		dxPacket->hwnd = dx_window()->m_hwnd;

		dxPacket->device = m_device;
		dxPacket->deviceContext = m_deviceContext;
	}
